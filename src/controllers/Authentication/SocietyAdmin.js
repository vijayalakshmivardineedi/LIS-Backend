...
exports.getAllSocieties = async (req, res) => {
    try {
        ...
        // Retrieve all societies from the database
        const societies = await SocietyAdmin.find();

        // Iterate over each society and update the status if needed
        for (const society of societies) {
            if (society.expiryDate && new Date(society.expiryDate) < currentDate) {
                society.active = false; // Set active status to false
                await society.save(); // Save the updated society to the database
            }
        }
        const society = await SocietyAdmin.find();
        // Return the updated list of societies
        res.status(200).json({
            societies: society
        });
    } catch (error) {
...
exports.profile = async (req, res) => {
    const { societyId } = req.params;
    try {
        const admins = await SocietyAdmin.findById( societyId );

        if (!admins || admins.length === 0) {
            return res.status(404).json({ message: "No admins found for this society" });
        }
