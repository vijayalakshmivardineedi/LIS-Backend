...
exports.getNotificationsBySocietyId = async (req, res) => {
    const { societyId } = req.params; // Get societyId from route parameters
    ...
    try {
        // Find notifications by societyId
        const notifyData = await Notification.find({ societyId });

        if (!notifyData.length) {
            return res.status(404).json({ message: "No notifications found for this societyId" });
        }
